...
module Trimble
  module AttributeHelper
	...
	PREF_NAMESPACE = 'AttributeHelper'
  ...
  class << self
	...
	def self.auto_open_enabled?
		Sketchup.read_default(PREF_NAMESPACE, AUTO_OPEN_KEY, true)
	end

	def self.set_auto_open(value)
		Sketchup.write_default(PREF_NAMESPACE, AUTO_OPEN_KEY, value)
	end
	...
	def self.visualize_selected
		...
		@window ||= UI::HtmlDialog.new(options)
...
def self.init_app_observer
  @app_observer ||= AppObserver.new
  Sketchup.remove_observer(@app_observer)
  Sketchup.add_observer(@app_observer)

  # Startup model
  return unless auto_open_enabled?
  ...
  class ModelObserver < Sketchup::ModelObserver
	...
	class AppObserver < Sketchup::AppObserver
		...
		def observe_model(model)
			return unless PLUGIN.auto_open_enabled?
	...
	unless file_loaded?(__FILE__)
		command = UI::Command.new("Attribute Helper") {
			self.visualize_selected
		}
		command.status_bar_text = "Inspect and edit the attributes of a selection."

		auto_open_cmd = UI::Command.new("Auto-open Attribute Helper") {
			self.set_auto_open(!self.auto_open_enabled?)
		}

		auto_open_cmd.set_validation_proc {
			self.auto_open_enabled? ? MF_CHECKED : MF_UNCHECKED
		}

		menu_name = Sketchup.version.to_f < 21.1 ? 'Plugins' : 'Developer'
		menu = UI.menu(menu_name)
		menu.add_item(command)
		menu.add_item(auto_open_cmd)

		self.init_app_observer
